...
//constants
float CLICKS_PER_REV = 627.2; //default = 627.2
...
void accelerateFlyWheel (float targetSpeed) //gradually increases speed of flywheel
{
	...
	float rightRPM;
	float lSpeed = 30;
	float rSpeed = 30; //both left and right speed have to start at 10 to begin the acceleration proccess
...
accelerateOn = (vexRT[Btn5D] == 1) ? true : false;
	...
	while(accelerateOn && (!lAtTarget || !rAtTarget)) //only accelerate if the appropriate button on the joystick is pressed and at least one side is not up to speed
	{
		...
		wait1Msec(1000);
		leftEncVal = nMotorEncoder[Fly2];
		rightEncVal = nMotorEncoder[Fly5];

		//convert encoder values to approximate RPM
		leftRPM = targetlRPM; //leftEncVal/CLICKS_PER_REV*60
		rightRPM = targetrRPM; //rightEncVal/CLICKS_PER_REV*60
		displayNextLCDNumber(0,331);
		//displayNextLCDNumber(1,331);
...
task main()
{
	...
	direction = vexRT[Btn7U] == 1 ? -1 : 1;
		...
		else if(vexRT[Btn8U] == 1) //8U sets speed to 75
		{
		setLeftSpeed(75);
		setRightSpeed(75);
		}
		else //stops wheels so they don't go forever...
		{
		setLeftSpeed(0);
		setRightSpeed(0);
		}
	...
	motor[intake1] = vexRT[Btn5U] == 1 ? 125*direction : 0;
	motor[intake2] = vexRT[Btn5D] == 1 ? -125*direction : 0;
	}
}
