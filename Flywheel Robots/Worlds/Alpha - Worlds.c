...
#pragma config(Motor,  port5,           rDriveMiddle,  tmotorVex393TurboSpeed_MC29, openLoop, reversed, encoderPort, I2C_4)
#pragma config(Motor,  port6,           lDriveFrontBack, tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port7,           rDriveFrontBack, tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
#pragma config(Motor,  port10,          intakeChain,   tmotorVex393TurboSpeed_HBridge, openLoop, encoderPort, I2C_3)
...
task drivetrainController() {
	...
	while(true) {
		...
		rYLastSent = rY;
		motor[lDriveFrontBack] = (abs(lY) > threshold) ? lY : 0;
		motor[lDriveMiddle] = (abs(lY) > threshold) ? lY : 0;
		motor[rDriveFrontBack] = (abs(rY) > threshold) ? rY : 0;
...
bool overrideAutoOuttakeOnly = false;
...
task autoIntake() {
	while(1) {
		...
		/*if (flywheelMode != 3 || flywheelMode != 4) { //don't auto-intake for purple or long shooting
			...
			} else if (SensorValue[intakeLimit] && rollerState == 0 && ballsInIntake > 0 && ballsInIntake <= 3) {
