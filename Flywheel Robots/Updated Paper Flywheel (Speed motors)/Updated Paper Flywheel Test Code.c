...
#pragma config(Motor,  port4,           flyWheelR1,    tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port5,           flyWheelR2,    tmotorVex393HighSpeed_MC29, openLoop, reversed, encoderPort, I2C_2)
...
task leftFwControlTask()
{
    ...
    while(1)
        ...
        //setLFly( fw->motor_drive );  don't do this - testing
...
task rightFwControlTask()
{
    ...
    while(1)
        ...
        // and finally set the motor control value
        //setRFly( fw->motor_drive ); //don't change this - testing only
        // Run at somewhere between 20 and 50mS
        wait1Msec( FW_LOOP_SPEED );
        }
}
...
void initializeTBH() {
	tbhInit(lFly, 392, 0.00735); //initialize TBH for left side of the flywheel
	tbhInit(rFly, 392, 0.00725); //initialize TBH for the right side of the flywheel
	//motor[intake] = 127;
...
task usercontrol()
{
	...
	setRFly(100);
	while(1) {
		writeDebugStreamLine("%d,%d",lFly.current, rFly.current);
	}
